...
module.exports = function setup(data) {
  ...
  */
  ...
  fs.writeFile(cacheFile, '{"word": [], "user": [], "playlist": [], "movie": [], "house": [], "deviceType": [], "room": [],"contact": []}', function(err) {
    if(err) {
        sails.log.error(err);
    }
    ...
    .then(() => {
          sails.log.info('Snips :: Injecting contacts slots...');
          return inject.contacts();
        })
    .then(() => {
