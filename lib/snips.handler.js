...
Traitement spécial
***/
console.log("snips message "+JSON.stringify(message));
if(message.intent.confidenceScore>0.25){
      if(topic.indexOf('intent') >= 0 && topic.indexOf('pushContactToGladysBrain') >= 0){
        ...
        message.input = "Mets à jour la date d'anniversaire du contact "+getSlotValue(message, "contact")
            +" dont la date est "+getSlotDateValue(message, "birthdate");
        ...
        message.input = "Mets à jour le numéro de téléphone du contact "+getSlotValue(message, "contact")
            +" dont le numéro est "+getSlotsValue(message, "phonenumber");
      }
      else if(topic.indexOf('intent') >= 0 && topic.indexOf('getContactAnnifToGladysBrain') >= 0){
        message.input = "Quelle est la date de naissance de contact "+getSlotValue(message, "contact")+"?"
      }
...
message.input = "Créé une alarme à ";
var results = chrono.parse(message.input);
  
  for(var i = 0; i < results.length; i++){
    message.input +=results[i].start.date()+" ";
  }
if(getSlotValue(message, "reason") && getSlotValue(message, "reason").length>0){
message.input += " pour goal "+getSlotValue(message, "reason")+".";
}
	...
	else if(topic.indexOf('intent') >= 0 && topic.indexOf('removeContactsFromGladysBrain') >= 0){
		  message.input = "Retire le contact "+getSlotValue(message, "contact") +" qui est sur la liste des contacts";
	      }
	else if(topic.indexOf('intent') >= 0 && topic.indexOf('GladysB') >= 0){
	console.log("snips intent "+topic);
var intentToTranscode=topic.split(':')[1];
		var transco=prop.get(intentToTranscode);
...
message.input="Je ne comprends pas";
}
