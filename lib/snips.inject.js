...
module.exports = {
  rooms: function() {
    ...
    contacts: function() {
        ...
        return gladys.modules.contacts.get.getContacts()
            .then((contacts) => {
                console.log("inject contacts "+JSON.stringify(contacts));
                contacts.forEach(function(contact) {
                    if(slots.indexOf(contact.contactName) === -1) {
                        slots.push(contact.contactName);
                    }
                });
                return shared.cacheSlots({"contact": slots})
                    .then((data) => {
                        return inject(data);
                    });
            });
